...
arrowNext.addEventListener('click', () => {
  ...
  if (activeSlide === slides[slides.length - 1]) {
    ...
    document.querySelector(`.${classActiveButton}`).classList.remove(classActiveButton);
  ...
  for (let i = 0; i < slides.length - 1; i++) {
    ...
    if(slides[i].classList.contains(classActiveSlide)) {
      ...
      document.querySelector(`.${classActiveButton}`).classList.remove(classActiveButton);
      buttons[i + 1].classList.add(classActiveButton);
      return;
    }
  }
});
...
arrowBack.addEventListener('click', () => {
  ...
  if (activeSlide === slides[0]) {
    ...
    document.querySelector(`.${classActiveButton}`).classList.remove(classActiveButton);
    buttons[slides.length - 1].classList.add(classActiveButton);
    return;
  }
